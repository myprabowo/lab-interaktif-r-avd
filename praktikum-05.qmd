...
::: {.callout-important}
## Apa itu Metode Konfirmasi?

**Metode konfirmasi** adalah pendekatan analisis yang dimulai dari **hipotesis yang sudah dirumuskan terlebih dahulu**, kemudian data dikumpulkan dan dianalisis untuk menguji apakah hipotesis tersebut didukung atau ditolak secara statistik.

Dalam praktikum ini, Anda akan:

- Merumuskan $H_0$ dan $H_1$ sebelum melihat data.
- Menghitung statistik uji (t-statistik, F-statistik) dan membandingkannya dengan nilai kritis.
- Mengambil keputusan berdasarkan p-value dan interval kepercayaan, bukan intuisi visual semata.

> **Perbandingan:** Jika metode eksplorasi (Praktikum 04) ibarat *detektif yang mencari petunjuk*, maka metode konfirmasi ini ibarat *hakim yang menguji bukti secara formal* menggunakan kriteria yang telah ditetapkan.
:::
...
- $H_0$: $\beta_1 = 0$: tidak ada hubungan linear antara prediktor dan respons
- $H_1$: $\beta_1 \neq 0$: ada hubungan linear yang signifikan

**Langkah-langkah uji hipotesis:**

1. **Tetapkan hipotesis** ($H_0$ dan $H_1$) dan tingkat signifikansi $\alpha$ (biasanya 0.05).
2. **Hitung statistik uji** — untuk koefisien regresi digunakan t-statistik: $t = \hat{\beta}_1 / SE(\hat{\beta}_1)$.
3. **Tentukan nilai kritis** atau hitung p-value.
4. **Buat keputusan**: tolak $H_0$ jika p-value $< \alpha$.
5. **Interpretasikan** dalam konteks masalah.
...
alpha <- 0.05
if (p_val < alpha) {
  cat("Keputusan  : Tolak H0 — wt berpengaruh signifikan terhadap mpg (p <", alpha, ")\n")
} else {
  cat("Keputusan  : Gagal tolak H0 — tidak cukup bukti pengaruh signifikan (p >=", alpha, ")\n")
}
```
...
### Latihan 1: Hitung Koefisien Manual
...
### Latihan 2: Verifikasi dengan `lm()`
...
### Latihan 3: Prediksi dan Interval Kepercayaan
...
### Latihan 4: Regresi Berganda + Diagnostik
